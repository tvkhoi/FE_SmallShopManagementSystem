...
export class CartService {
  private readonly baseUrl = 'https://localhost:7277/api/Cart';
  private readonly cartChangedSource = new BehaviorSubject<void>(undefined);
  ...
  getCartItemMainImage(item: CartItem): string {
    return item.imageUrls && item.imageUrls.length > 0 ? item.imageUrls[0] : 'assets/default.jpg';
  }
  ...
  removeFromCart(productId: number): Observable<ApiResponse<string>> {
    return this.http
      .delete<ApiResponse<string>>(`${this.baseUrl}/${productId}`)
